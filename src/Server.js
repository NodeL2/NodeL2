class Server {
    constructor(name, optn, callback) {
        ...
        require('net').createServer(this.onSocket.bind(parameters)).listen(optn.port, optn.hostname, () => {
            console.info('%s:: successful init for %s:%d'.ok, name, optn.hostname, optn.port);
        });
    }

    onSocket(socket) {
        console.info(
            '%s:: new connection received from %s:%d'.ok, this.name, socket.remoteAddress, socket.remotePort
        );
